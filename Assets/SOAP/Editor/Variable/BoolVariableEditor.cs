...
namespace SOAP
{
    ...
    public class BoolVariableEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // --- Initial / Reset Value ---
            EditorGUI.BeginChangeCheck();
            bool newInitial = EditorGUILayout.Toggle("Initial (Reset) Value", script.initialValue);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            // --- Live Runtime Values ---
            GUI.enabled = false;
            EditorGUILayout.Toggle("Current (Runtime) Value", script.currentValue);
            EditorGUILayout.Toggle("Previous (Runtime) Value", script.previousValue);
            GUI.enabled = true;
